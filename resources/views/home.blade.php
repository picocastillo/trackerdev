...
@section('content')
    ...
    <div class="my-2 flex flex-wrap items-center gap-2">
        ...
        <form id="form_filter_by_project" onchange="onChangeFilterByProject()" class="flex items-center" action="/home" method="GET">
            {{ csrf_field() }}
            <select name="project_id" class="form-input ml-1 w-auto">
        ...
        @if (isManager())
            <form class="flex items-center" id="form_filter_by_user" onchange="onChangeFilterByUser()" action="/home" method="GET">
                {{ csrf_field() }}
                <select name="user_id" class="form-input ml-1 w-auto">
        ...
        @if (isDeveloper())
            <button type="button" class="btn btn-primary" onclick="document.getElementById('ChargeTime').showModal()">
                Cargar Tiempo
            </button>
